...
class VerifyPin extends StatefulWidget {
  ...
  @override
  State<StatefulWidget> createState() => _VerifyPinPageState(onDone: onDone);
}
...
class _VerifyPinPageState extends State<VerifyPin> {
  ...
  _VerifyPinPageState({@required this.onDone});
  ...
  void loginIfPinCorrect(int currentAttempts) async {
    ...
    if (pin.length == PIN_LENGTH) {
      ...
      if (wasSame) {
        ...
        Toast.show(
            "Invalid Pin. \n"
                "You have ${PinConstants.ALLOWED_ATTEMPTS -
                currentAttempts} attempts remaining",
            context);
        setState(() {
          pin = "";
        });
  ...
  void pinVerification() async {
    ...
    if (currentAttempts < PinConstants.ALLOWED_ATTEMPTS) {
      ...
      if (delta < PinConstants.RETRY_INTERVAL) {
        Toast.show(
            "Your wallet is locked for ${TimeUtil.getMinutesSeconds(
                ((PinConstants.RETRY_INTERVAL - delta) / 1000)
                    .ceil())} minutes since last attempt, try again later",
            context);
        if (_timer != null || !_timer.isActive) {
          startTimer(((PinConstants.RETRY_INTERVAL - delta) / 1000).ceil());
        }
      } else {
        userRepository.updateIncorrectAttempt(attempts: 0, time: null);
        loginIfPinCorrect(0);
      }
      setState(() {
        pin = "";
  ...
  Widget build(BuildContext context) {
    var scaffold = Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text("Unlock your wallet", style: TextStyle(color: Colors.white)),
          centerTitle: true,
        ),
                      ...
                      child: Center(
                        child: Text(
                          _timer != null && _timer.isActive
                              ? "Try again in ${TimeUtil.getMinutesSeconds(
                              _start)} minutes"
                              :
